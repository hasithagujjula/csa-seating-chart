...
public class GameGUI extends JComponent
{
  ...
  // walls, prizes, traps, and lollipops
  ...
  private Image lollipopImage;
  private int totalLollipops; // extra feature: collecting a lollipop adds more prizes and traps to the board and increases timer by 10 seconds
  private Rectangle[] lollipops;
  ...
  public GameGUI()
  {
    ...
    try {
      prizeImage = ImageIO.read(new File("candy.png"));      
    } catch (Exception e) {
      System.err.println("Could not open file candy.png");
    }
    try {
      lollipopImage = ImageIO.read(new File("lollipop.png"));      
    } catch (Exception e) {
      System.err.println("Could not open file lollipop.png");
    }
  ...
  */
  public void createBoard()
  {
    ...
    lollipops = new Rectangle[totalLollipops];
    createLollipops();
  }

  // Adding another feature to the game where if the player picks up a specific prize, more prizes and traps are added to the board and the timer is increased by 20 seconds
  public void addMorePrizesAndTraps(){
    createPrizes();
    createTraps();
    createLollipops();
    repaint();
  ...
  public int movePlayer(int incrx, int incry)
  {
      ...
      {
        System.out.println ("OFF THE GRID! Score deducted by " + offGridVal + " points.");
        return -offGridVal;
      }
      ...
      for (Rectangle r: walls)
      {
        ...
        {
          System.out.println("A WALL IS IN THE WAY. Score deducted by " + hitWallVal + " points.");
          return -hitWallVal;
        }
  ...
  // function to check if there is a lollipop where the player will land
  public boolean isLollipop(int newx, int newy)
  {
    double px = playerLoc.getX() + newx;
    double py = playerLoc.getY() + newy;

    for (Rectangle r: lollipops)
    {
      // DEBUG: System.out.println("lollipopx:" + r.getX() + " lollipopy:" + r.getY() + "\npx: " + px + " py:" + py);
      // zero size lollipops have already been picked up, ignore
      if (r.getWidth() > 0)
      {
        // if new location of player has a lollipop, return true
        if (r.contains(px, py))
        {
          return true;
        }
  ...
  public int springTrap(int newx, int newy)
  {
    ...
    for (Rectangle r: traps)
    {
      ...
      if (r.contains(px, py))
      {
        ...
        {
          r.setSize(0,0);
          System.out.println("TRAP IS SPRUNG! Score increased by " + trapVal + " points.");
          return trapVal;
        }
      }
    }
    // no trap here, penalty
    System.out.println("THERE IS NO TRAP HERE TO SPRING. Score deducted by " + trapVal + " points.");
    return -trapVal;
  }
  ...
  public int pickupPrize()
  {
    ...
    for (Rectangle p: prizes)
    {
      ...
      {
        System.out.println("YOU PICKED UP A PRIZE! Score increased by " + prizeVal + " points.");
        p.setSize(0,0);
        repaint();
        return prizeVal;
      }
    }
    System.out.println("OOPS, NO PRIZE HERE. Score deducted by " + prizeVal + " points.");
    return -prizeVal;  
  }
  ...
  */
  ...
  public void paintComponent(Graphics g) {
    ...
    // add lollipops
    for (Rectangle l : lollipops)
    {
      // picked up lollipops are 0 size so don't render
      if (l.getWidth() > 0) 
      {
      int lx = (int)l.getX();
      int ly = (int)l.getY();
      g.drawImage(lollipopImage, lx, ly, null);
      }
    }
  ...
  // function for creating lollipops
  private void createLollipops()
  {
    int s = SPACE_SIZE; 
    Random rand = new Random();
     for (int numLollipops = 0; numLollipops < totalLollipops; numLollipops++)
     {
      int h = rand.nextInt(GRID_H);
      int w = rand.nextInt(GRID_W);
  ...
  private int playerAtEnd() 
  {
    ...
    {
      System.out.println("YOU MADE IT! Score increased by " + endVal + " points.");
      score = endVal;
    }
    else
    {
      System.out.println("OOPS, YOU QUIT TOO SOON! Score deducted by " + endVal + " points.");
      score = -endVal;
    }
    return score;
